...
tally_delimited_string <-
  ...
  function(x, col, delim = ",",
    ...
    ided <- dplyr::mutate(x, carutools_tds_id = dplyr::row_number())

    separated_answers <-
      ided |>
      dplyr::select(carutools_tds_id, !!col) |>
      tidyr::separate_longer_delim(!!col, delim = delim) |>
      dplyr::group_by(carutools_tds_id, !!col) |>
      dplyr::count()
    ...
    if(!is.null(keep)){
      separated_answers <-
        dplyr::group_by(separated_answers, carutools_tds_id) |>
      ...
      other_data <-
        dplyr::select(separated_answers, carutools_tds_id, !!other_col, !!tally_col) |>
        dplyr::distinct() |>
      ...
      separated_answers <-
        dplyr::select(separated_answers, carutools_tds_id, !!col, n) |>
    ...
    if(!is.null(keep)){
      out <-
        dplyr::left_join(out, other_data, by = "carutools_tds_id")
    }

    out <- dplyr::select(out, -carutools_tds_id)
